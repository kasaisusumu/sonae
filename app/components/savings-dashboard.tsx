...
/** ホームに表示する「防げた件数」ダッシュボード。防げたことも並べる。 */
export async function SavingsDashboard({ userId }: { userId: string }) {
  ...
  const maxCategory = Math.max(1, ...s.byCategory.map((c) => c.count));
  ...
  // まだ「防げた」記録が無いあいだは、件数欄に「例」を出す。
  // 本物の記録が1件でも入ったら hasAny が true になり、例は自動で消える。
  const hasAny = s.entryCount > 0;
  const EXAMPLE_COUNT = 3; // 表示専用のダミー。DB・集計には一切入らない。

  return (
    <section className="space-y-3">
      <div className="flex items-baseline justify-between">
        <h2 className="text-lg font-semibold">うっかり、いくつ防げた？</h2>
        <Link
          href="/failures"
      ...
      <div className="rounded-2xl bg-foreground px-6 py-6 text-surface">
        {hasAny ? (
          <>
            <p className="text-sm text-surface/70">今月、防げた件数</p>
            <p className="mt-1 text-4xl font-bold">{s.thisMonthCount}件</p>
            <p className="mt-1.5 text-xs text-surface/70">
              防げたうっかり {retro.totalCount} 件 ・ 累計 {s.entryCount} 件
            </p>
          </>
        ) : (
          <>
            ...
            <p className="mt-1 text-4xl font-bold">{EXAMPLE_COUNT}件</p>
        ...
        <p className="mt-3 text-[11px] text-surface/55">
          ※ 金額ではなく件数で数えます（「防げた」と選んだ回数の合計・自動判定なし）
        </p>
      </div>
                        ...
                        <li key={it.id} className="py-2">
                          ...
                          <div className="mt-1.5 flex flex-wrap items-center gap-3 text-[11px]">
                            <form action={setFailureOutcome}>
                              ...
                              />
                              <ConfirmButton
                                message="「防げた」の計上を取り消しますか？（防げた件数から外れます）"
                            ...
                            <form action={setFailureOutcome}>
                              ...
                              />
                              <ConfirmButton
                                message="「防げなかった」に変更しますか？（防げた件数から外れます）"
                            ...
                            <form action={setFailureOutcome}>
                              ...
                              />
                              <ConfirmButton
                                message="「今回は関係ない」に変更しますか？（防げた件数から外れます）"
                                className="text-muted underline hover:text-foreground"
                              >
                                今回は関係ない
                              </ConfirmButton>
